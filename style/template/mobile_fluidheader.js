...
function doResize()
{
    var top = Math.floor(window.pageYOffset),
        header = document.getElementById('page-header');
    if(top > 0)
    {
        header.style.top = top + 'px';
        if(header.className.indexOf(' moved') < 1)
        {
            header.className += ' moved';
        }
    }
    else
    {
        header.style.top = 0;
        if(header.className.indexOf(' moved') > 0) header.className = header.className.replace(' moved', '');
    }
    var i, list;
